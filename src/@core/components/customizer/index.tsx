...
interface CustomizerProps {
  ...
  setLastLayout: (layout: string) => void
...
const Customizer = ({
  ...
  const renderSkinsRadio = () => {
    const skinsArr = [
      { name: 'light', label: 'Light', checked: skin === 'light' },
      { name: 'bordered', label: 'Bordered', checked: skin === 'bordered' },
      { name: 'dark', label: 'Dark', checked: skin === 'dark' },
      { name: 'semi-dark', label: 'Semi Dark', checked: skin === 'semi-dark' }
    ]
  ...
  const renderNavbarTypeRadio = () => {
    const navbarTypeArr = [
      { name: 'floating', label: 'Floating', checked: navbarType === 'floating' },
      { name: 'sticky', label: 'Sticky', checked: navbarType === 'sticky' },
      { name: 'static', label: 'Static', checked: navbarType === 'static' },
  ...
  const renderFooterTypeRadio = () => {
    const footerTypeArr = [
      { name: 'sticky', label: 'Sticky', checked: footerType === 'sticky' },
      { name: 'static', label: 'Static', checked: footerType === 'static' },
      { name: 'hidden', label: 'Hidden', checked: footerType === 'hidden' }
    ]
    return footerTypeArr.map((radio, index) => {
      const marginCondition = index !== footerTypeArr.length - 1
      return (
        <div key={index} className={classnames('form-check', { 'mb-2 me-1': marginCondition })}>
          <Input type='radio' id={`footer-${radio.name}`} checked={radio.checked} onChange={() => setFooterType(radio.name)} />
  ...
  return (
    <div className={classnames('customizer d-none d-md-block', { open: openCustomizer })}>
        ...
        <div className='px-2'>
          ...
          {layout !== 'horizontal' ? (
            <div className='form-switch mb-2 ps-0'>
              <div className='d-flex align-items-center'>
                ...
                <Input type='switch' id='menu-collapsed' name='menu-collapsed' checked={menuCollapsed} onChange={() => setMenuCollapsed(!menuCollapsed)} />
              </div>
            </div>
          ) : null}
          ...
          <div className='form-switch mb-2 ps-0'>
            <div className='d-flex align-items-center'>
              ...
              <Input type='switch' id='menu-hidden' name='menu-hidden' checked={isHidden} onChange={() => setIsHidden(!isHidden)} />
            </div>
          </div>
        </div>
